...
destination="$root/plugins/rf-dls"
...
test -f "$package/lib/librackforge_rf_dls.so"
grep -qx 'id = "org.rackforge.rf-dls"' "$package/rackforge-plugin.toml"

install -d "$root/plugins" "$root/backups"
stage="$(mktemp -d "$root/plugins/.rf-dls-stage.XXXXXX")"
backup="$root/backups/rf-dls-$(date -u +%Y%m%dT%H%M%SZ)-$$"
cleanup() { rm -rf "$stage"; }
trap cleanup EXIT
cp -a "$package/." "$stage/"
